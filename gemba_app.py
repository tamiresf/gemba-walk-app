...
with aba3:
    ...
    if df_rot_exibir.empty:
        ...
        for idx_r, (r_idx, row_r) in enumerate(df_rot_exibir.iterrows()):
            ...
            id0_rot = str(row_r.get("id0", str(r_idx)))
            sp_id_rot = str(row_r.get("id", str(r_idx)))
            ...
            with cols_r[idx_r % 2]:
                with st.container(border=True):
                    # Cabeçalho com o ícone de Lixeira ao lado do título
                    ...
                    if st.session_state.get(f"confirm_delete_{id0_rot}_{r_idx}", False):
                        st.warning("Tem certeza que deseja excluir esta rotina?")
                        col_c1, col_c2 = st.columns(2)
                        with col_c1:
                            if st.button("Sim, Excluir", key=f"sim_del_{id0_rot}_{r_idx}", type="primary"):
                                payload_excluir = {"id0": id0_rot, "id": sp_id_rot}
                                
                                # 1. Excluir no Power Automate / SharePoint
                                if WEBHOOK_ROTINAS_EXCLUIR:
                                    try:
                                        requests.post(WEBHOOK_ROTINAS_EXCLUIR, json=payload_excluir, timeout=10)
                                    except Exception as e:
                                        st.error(f"Erro ao excluir no SharePoint: {e}")

                                # 2. Excluir do estado local se existir
                                if "rotinas_local" in st.session_state and not st.session_state["rotinas_local"].empty:
